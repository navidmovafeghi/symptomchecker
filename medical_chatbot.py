...
def create_nodes(model):
    """Create all nodes with the given LLM model."""
...
Respond with ONLY one word: CONTINUATION, TOPIC_SWITCH, UNCLEAR, EXIT, or FRUSTRATED"""
        ...
        if "TOPIC_SWITCH" in response_type:
            # Reset symptom history and go back to intent detection
            return Command(
        ...
        if "FRUSTRATED" in response_type or unclear_count >= 2:
            # Too many unclear responses or frustrated - give early answer
            return Command(
        ...
        gather_prompt = f"""You are a professional medical assistant gathering symptom information.
...
Your task:
1. Analyze what symptom information has been gathered so far
2. Generate ONE focused follow-up question to gather more specific information about:
...
Ask only ONE clear, specific question. Be empathetic and professional."""
        ...
        return Command(
            update={
                ...
                "unclear_count": 0  # Reset on good response
            },
    ...
    def symptom_clarification_node(state: MedicalChatState) -> Command[Literal["wait_for_symptom_clarification"]]:
        ...
        if unclear_count >= 2:
            # Offer options after multiple unclear responses
...
# ============== GRAPH ==============
def create_medical_chatbot(model):
    ...
    # Add all nodes
    builder.add_node("intent_detector", nodes["intent_detector"])
    builder.add_node("clarification_node", nodes["clarification_node"])
    builder.add_node("wait_for_clarification", nodes["wait_for_clarification"])
    builder.add_node("non_medical_response", nodes["non_medical_response"])
    builder.add_node("symptom_checker", nodes["symptom_checker"])
    builder.add_node("symptom_clarification_node", nodes["symptom_clarification_node"])
    builder.add_node("wait_for_symptom_clarification", nodes["wait_for_symptom_clarification"])
    builder.add_node("polite_end_node", nodes["polite_end_node"])
    builder.add_node("symptom_evaluator", nodes["symptom_evaluator"])
    builder.add_node("wait_for_response", nodes["wait_for_response"])
    builder.add_node("final_answer", nodes["final_answer"])
    builder.add_node("other_medical_response", nodes["other_medical_response"])
...
graph = create_medical_chatbot(model)
